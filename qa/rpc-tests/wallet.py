...
#   c) node1 mines 101 blocks, so now nodes 0 and 1 have 500 CURIUM, node2 has none. 
#   d) node0 sends 21 CURIUM to node2, in two transactions (11 CURIUM, then 10 CURIUM).
...
class WalletTest (BitcoinTestFramework):
    ...
    def check_fee_amount(self, curr_balance, balance_with_fee, fee_per_byte, tx_size):
        ...
        if fee < target_fee:
            raise AssertionError("Fee of %s CURIUM too low! (Should be %s CURIUM)"%(str(fee), str(target_fee)))
        # allow the node's estimation to be at most 2 bytes off
        if fee > fee_per_byte * (tx_size + 2):
            raise AssertionError("Fee of %s CURIUM too high! (Should be %s CURIUM)"%(str(fee), str(target_fee)))
        return curr_balance
...
<<<<<<< HEAD
        # Send 21 CURIUM from 0 to 2 using sendtoaddress call.
=======
        ...
        # Send 210 CURIUM from 0 to 2 using sendtoaddress call.
>>>>>>> dev-1.12.1.0
        ...
        # node0 should end up with 1000 CURIUM in block rewards plus fees, but
<<<<<<< HEAD
...
=======
        ...
        # Send 100 CURIUM with subtract fee from amount
        ...
        # Sendmany 100 CURIUM with subtract fee from amount
