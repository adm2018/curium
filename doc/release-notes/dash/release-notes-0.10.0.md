...
- Darksend tested with 2.5 CRU successfully and I've set that as the minimum
...
- Fixed the endless splitting issue causes by splitting 1000CRU and not making a DS compatible input
...
- Fixed issue with denominating small amounts of CRU in large wallet
  (http://jira.darkcoin.qa/browse/CRU-46)
- Made splitting up initial inputs much more efficient. Now when splitting up,
  it will use powers of two from 4096 CRU in reverse to get the best possible
...
- Fixed issue with completed amount jumping around
  (http://jira.darkcoin.qa/browse/CRU-46)
- Made messages less threatening (http://jira.darkcoin.qa/browse/CRU-60)
...
- Darksend limited to 5000CRU per wallet.dat. Client will warn about this the
  first time it's opened, then disable darksend from then on.
- Fixed some bugs with connecting to the correct masternodes
- Send was sending way too many coins for all modes, (I sent 100CRU anon and it
  sent 2000CRU, then sent me change for the rest causing a whole reprocess of
...
- Fees per round of Darksend are 0.001CRU or $0.00538 at current prices. This
  means to anonymize 1000CRU with 3 rounds (an average use case) it would cost
  a user 1.5 cents.
...
- RPC calls are changed a bit:
    ...
    darksend denominate 100 - Will denominate 100CRU
    darksend auto - Will run AutoDenominate
    darksend Xaddr 100 - Will send 100 denominated CRU to Xaddr
    sendtoaddress Xaddr 100 - Will send 100 non-denominated CRU to Xaddr
...
- Added a smart input splitting method. Place 1000+CRU into a brand new wallet
...
- Mixing rounds are limited to 1000CRU, per block
...
- DoAutomaticDenominations 100CRU limit changed to 500CRU (we should see a bunch
