...
*Setup instructions for a gitian build of Curium using a Debian VM or physical system.*
...
- [Building Curium](#building-curium)
...
Clone the git repositories for curium and gitian and then checkout the curium version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the curium repository
...
To build Curium (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the curium repository.
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/curium-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/curium-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/curium-build.assert
```
