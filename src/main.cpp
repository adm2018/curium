...
/** Fees smaller than this (in crus) are considered zero fee (for relaying, mining and transaction creation) */
                              ...
                              std::vector<uint256>& vHashTxnToUncache, bool fDryRun)
{
...

    {
        ...
        if (fRejectAbsurdFee && nFees > ::minRelayTxFee.GetFee(nSize) * 1000)
            return state.Invalid(false,
                REJECT_HIGHFEE, "absurdly-high-fee",
                strprintf("%d > %d", nFees, ::minRelayTxFee.GetFee(nSize) * 1000));
